...
userSchema.pre('save', async function (next) { // we use regular function because the 'this' binding is needed
    const user = this
    
    if (user.isModified('password')) { // true when user is created and when user's password is updated
